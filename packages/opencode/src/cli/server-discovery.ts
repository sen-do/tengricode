...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const remove: Interface["remove"] = Effect.fn("CliServerDiscovery.remove")(function* () {
    ...
    const write: Interface["write"] = Effect.fn("CliServerDiscovery.write")(function* (url) {
      yield* fs.writeJson(file, { url: localURL(url).toString(), pid: process.pid }, 0o600).pipe(Effect.orDie)
    })

    const findLocal: Interface["find"] = Effect.fn("CliServerDiscovery.find")(function* () {
      const entry = yield* read()
